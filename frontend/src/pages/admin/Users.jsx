...
import { authAPI, handleApiError, getStatusBadgeClass, getRoleBadgeClass, formatDateTime } from "../../services/api";
...
const Users = () => {
  ...
  const handleUpdateStatus = async (status) => {
    ...
    try {
      setActionLoading(true);
      const response = await authAPI.updateUserStatus(selectedUser.user_id, status);
      if (response.success) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage all users, view details, and control access.</p>
          </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4">
                <div>
                  ...
                  <span className={`inline-block mt-1 px-3 py-1 text-xs font-medium rounded-full ${getRoleBadgeClass(selectedUser.role)}`}>
                    {selectedUser.role}
                  </span>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Status</p>
                  <span className={`inline-block mt-1 px-3 py-1 text-xs font-medium rounded-full capitalize ${getStatusBadgeClass(selectedUser.status)}`}>
                    {selectedUser.status}
                  </span>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Phone</p>
                  <p className="mt-1 font-medium text-gray-900">{selectedUser.phone || "Not provided"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">User ID</p>
                  <p className="mt-1 font-medium text-gray-900">#{selectedUser.user_id}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm text-gray-500">Joined</p>
                  <p className="mt-1 font-medium text-gray-900">{formatDateTime(selectedUser.created_at)}</p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full">
            <div className="p-6 border-b">
              ...
              <p className="text-gray-500 mt-1">Update status for {selectedUser.full_name}</p>
            </div>
            <div className="p-6 space-y-3">
              <p className="text-sm text-gray-600 mb-4">
                Current status: <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getStatusBadgeClass(selectedUser.status)}`}>{selectedUser.status}</span>
              </p>
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full">
            <div className="p-6">
              ...
              <h2 className="text-xl font-bold text-gray-900 text-center">Delete User</h2>

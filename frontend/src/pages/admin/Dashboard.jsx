...
const Dashboard = () => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: UserCheck,
      colorClass: "bg-[#ef4444] dark:bg-[#dc2626]",
    },
  ];
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                    <tr>
                      <th className="text-left px-6 py-4 text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                        User
                      </th>
                      <th className="text-left px-6 py-4 text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                        Role
                    ...
                    {recentUsers.length === 0 ? (
                      ...
                      recentUsers.map((user) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`px-3 py-1 text-[11px] font-black rounded-full uppercase tracking-widest ${getRoleBadgeClass(user.role)}`}
                            >
                              {user.role}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <span
                              className={`px-3 py-1 text-[11px] font-black rounded-full uppercase tracking-widest ${getStatusBadgeClass(user.status)}`}
                            >
                              {user.status}
                            </span>
                          </td>
            ...
            <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-gray-700/50 overflow-hidden shadow-lg divide-y divide-gray-100 dark:divide-gray-800/50">
              {pendingApprovals.length === 0 ? (
                ...
                pendingApprovals.slice(0, 5).map((user) => (
                  <div
                    key={user.user_id}
                    className="p-5 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span
                        className={`px-2 py-0.5 text-[10px] font-black rounded-full uppercase tracking-widest ${getRoleBadgeClass(user.role)}`}
                      >
                        {user.role}
                      </span>
                    </div>
